...
fn test_perfect_squares() {
    ...
    for &n in &squares {
        assert!(is_probably_square(&Integer::from(n)), "Expected {} to be a perfect square", n);
    }
}
...
fn test_non_perfect_squares() {
    ...
    for &n in &non_squares {
        assert!(!is_probably_square(&Integer::from(n)), "Expected {} to not be a perfect square", n);
    }
}
